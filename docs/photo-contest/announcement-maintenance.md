...
| 種類 | 場所 |
|---|---|
| コンテンツ（文章・状態・SNS・協賛・景品など） | `public/data/photo-contests/announcements/2026-01.json` |
| 画像（メインビジュアル） | `public/images/photo-contests/2026-01/announcement/` |
| 画像（景品） | `public/images/photo-contests/2026-01/prizes/` |

一覧ページ（`/photo-contest`）からの導線は `public/data/photo-contests/archive.json` で管理します（→ 16章）。

JSONは「キー」と「値」の集まりです。**値（`"..."` の中身）だけ**を書き換えてください。
カンマ `,` や括弧 `{ } [ ]` を消すとページが表示されなくなります。編集後は必ず 18章の確認を行ってください。
...
## 13. 景品の追加・削除・並び替え・画像設定

「協賛ブランド」の下、ページ最下部のフッターの手前に表示される「フォトコンテスト景品」セクションです。
写真とブランド名・景品名・紹介文を、1件ずつ縦に並べて表示します。

```json
"prizes": {
  "eyebrow": "PRIZES",
  "title": "フォトコンテスト景品",
  "sourceLinkLabel": "VIEW ON X",
  "items": [
    {
      "id": "prize-01",
      "order": 1,
      ...
      "images": [
        { "src": "/images/photo-contests/2026-01/prizes/prize-01/01.jpg", "alt": "画像の内容を説明する文字" },
        { "src": "/images/photo-contests/2026-01/prizes/prize-01/02.jpg", "alt": "画像の内容を説明する文字" }
      ],
      "imageLayout": "inset",
      "sourceUrl": "https://x.com/r_outdoor_photo/status/2085665083288600750",
      "isPublished": true
    }
  ]
}
...
| キー | 意味 |
|---|---|
| `eyebrow` | セクション見出し上の小さな英字ラベル（例 `"PRIZES"`） |
| `title` | セクションの日本語見出し（例 `"フォトコンテスト景品"`） |
| `sourceLinkLabel` | 各景品の下に出るXリンクの文言（例 `"VIEW ON X"`） |
| `items` | 景品1件ごとの配列 |
...
| キー | 意味 |
|---|---|
| `id` | 景品のID。他と重複しない英数字（例 `"prize-01"`） |
| `order` | 表示順（小さいほど先。重複させない） |
| `brand` | ブランド名 |
| `name` | 景品名（最も大きく表示されます） |
| `description` | 紹介文。**1つの文字列＝1段落**として配列で指定します（段落を分けたくなければ1要素だけでも構いません） |
| `images` | 画像の配列。**1枚〜複数枚**に対応（詳しくは次項） |
| `imageLayout` | 画像が2枚のときの並べ方。`"inset"`（重ねる／省略時もこれ）か `"pair"`（左右に並べる）。**省略可** |
| `sourceUrl` | 紹介元のX投稿URL（`https://x.com/...`）。**有効なURLが必須です** |
| `isPublished` | `false` にするとその景品を非表示にできます |

### 景品の追加

`{ ... }` のかたまりをコピーして値を書き換え、`id` と `order` を他と重複しない値にします。かたまりの間はカンマ `,` で区切ります。

### 景品の削除

不要な `{ ... }` のかたまりを（前後のカンマも含めて）消します。画像フォルダ自体は残しておいても表示には影響しません。

### 表示順の変更

各項目の `order` の数字を書き換えます（小さいほど先に表示されます）。

### ブランド名・景品名・紹介文の変更

`brand` / `name` / `description` の値を書き換えます。`description` は文章の区切りたい箇所で要素を分けると、Web上で段落として表示されます。

### 画像の追加・削除・差し替え

- 画像は `public/images/photo-contests/2026-01/prizes/prize-0N/`（`N` は景品番号）に置きます。
- **1枚だけのとき**：`images` に1件だけ指定すると、大きな1枚画像として表示されます。
- **2枚のとき**：`imageLayout` で並べ方を選べます（3枚目以降を指定しても画面には反映されません。2枚目までが使われます）。
  - `"inset"`（または `imageLayout` を書かない）：1枚目が大きなメイン画像、2枚目がその手前に重なる小さな添え画像になります。**同じ景品の別カット**（全体写真＋ディテール写真など）向けです。
  - `"pair"`：2枚を左右に等分で並べます。**別々の製品を2点プレゼントする景品**（手ぬぐい＋ステッカー、ボトル2種など）のように、2枚を対等に見せたいとき向けです。スマホでも横2列のまま表示されます。
  - 迷ったら「1枚目と2枚目が同じ物なら `inset`、違う物なら `pair`」と考えてください。
- **差し替え**：同じファイル名で新しい画像を上書きするか、新しいファイル名を使う場合は `src` の値も合わせて書き換えます。
- **alt**：画像の内容を説明する文字です（目が見えない方や読み上げ用）。「〇〇ブランドの△△の写真」のように、実際の景品名が分かる形で必ず入れてください。

#### 推奨画像サイズ

| 用途 | 推奨サイズ | 形式 |
|---|---|---|
| メイン画像（1枚目） | 長辺1600px程度 | JPG |
| 添え画像（2枚目・`inset`） | 長辺1000px程度 | JPG |
| 並列2枚目（`pair`） | 1枚目と同じく長辺1600px程度 | JPG |

- `pair` の2枚は同じ大きさで表示されます。**PCは縦長（4:5）、スマホは正方形（1:1）に自動で切り抜かれる**ので、被写体は中央寄りに配置した写真を選んでください。

- 1枚あたり **250KB 以下** を目安に書き出してください。
- 元の写真を無理に引き伸ばしたり、粗く圧縮しすぎたりしないでください。

### 元X投稿URLの変更

`sourceUrl` の値を書き換えます。景品ごとの「VIEW ON X」リンクの遷移先になります。`"#"` や `example.com` などの仮リンクは絶対に入れないでください。

### 景品を非公開にする

その項目の `isPublished` を `false` にします。公開中の景品が0件になると、セクション自体が表示されなくなります（右側のスマホ用ナビの番号も自動的に詰まります）。
...
## 15. 新しい開催回を追加する（例：2026年 第2回 / 2027年）
...
## 16. 一覧ページ（/photo-contest）からのリンク設定
