...
クラスタマップは全93件を1投稿＝1点で配置したもの（点の大きさ＝いいね数、`generate_cluster_map.py` で再生成可能）。点の色分けはキーワードルールによる自動分類のため、下記の手動クラスタリングとは件数が多少ずれる（例: 引用リポストで感想が本文に含まれる投稿は自動分類だと「感想・絶賛」に寄る）。ネタ枠がわずか7件で全いいねの過半を稼いでいるのが視覚的にわかる。
