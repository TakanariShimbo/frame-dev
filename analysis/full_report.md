# ヤマフレーム Xポスト フル解析レポート（94件 / 2026-08-01時点）

データ: `yamaframe_x_posts_2026-08-01.csv`
収集方法: Yahoo!リアルタイム検索（「#ヤマフレーム」77件＋タグなし「ヤマフレーム」の新規15件＋個別確認・引用元スイープで発見した2件）

## サマリー

- リリース（7/23）から約1週間で94件の言及、批判ゼロ。ユーザー数は運営発表で 24hで700人 → 2日で1000人 → 1週間で1700人。
...
クラスタマップは全94件を1投稿＝1点で配置したもの（点の大きさ＝いいね数、`generate_cluster_map.py` で再生成可能）。点の色分けはキーワードルールによる自動分類のため、下記の手動クラスタリングとは件数が多少ずれる（例: 引用リポストで感想が本文に含まれる投稿は自動分類だと「感想・絶賛」に寄る）。ネタ枠がわずか7件で全いいねの過半を稼いでいるのが視覚的にわかる。
...
3. **感想・絶賛**（約18件）
4. **ネタ・大喜利 #間違った使い方**（約7件）
   猫動画2,781いいね・猫の「山頂超え」835いいね（全体1・2位）、グラビア119いいね。運営の「絶対にやるなよ？」公認が拡散を後押し。
5. **用途の拡張発見**（約8件）
