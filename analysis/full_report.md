# ヤマフレーム Xポスト フル解析レポート（92件 / 2026-08-01時点）
...
- リリース（7/23）から約1週間で92件の言及、批判ゼロ。ユーザー数は運営発表で 24hで700人 → 2日で1000人 → 1週間で1700人。
- いいねトップは「間違った使い方」ネタ（猫の山頂超え835いいね）。バズはネタ枠、継続利用は山行記録と用途拡張が支える二層構造。
...
| 37 | Ryo Takano | 蝶ヶ岳の朝 |
| 30 | （引用）@negiasa | 「Photoshopいじるまでもない手軽さ」 |
...
クラスタマップは全92件を1投稿＝1点で配置したもの（点の大きさ＝いいね数、`generate_cluster_map.py` で再生成可能）。点の色分けはキーワードルールによる自動分類のため、下記の手動クラスタリングとは件数が多少ずれる（例: 引用リポストで感想が本文に含まれる投稿は自動分類だと「感想・絶賛」に寄る）。ネタ枠がわずか6件で全いいねの4割超を稼いでいるのが視覚的にわかる。
